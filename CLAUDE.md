...
5. **CuRobo V2 与 V1 接口差异较大，必须以实际代码为准** — 文档中的接口仅作参考，实施前必须检索 `third_party/curobo/` 源码确认类名、方法签名和参数格式，不得仅凭 V1 经验推测
6. **每阶段完成后必须输出工作汇报** — 包括完成的工作、遇到的问题、问题的解决方式
7. **每阶段完成后必须做回归测试** — 完成新功能后，必须从头执行完整程序链路（从配置加载到最终输出），确保已有功能未被破坏。测试应覆盖：(1) 新功能本身；(2) 之前阶段已通过的测试用例；(3) 边界情况（空障碍物、默认配置等）。发现回归问题必须立即修复后再继续
